...
class DataExchangeUtil {
     ...
     */
    getFileAbsPath(fileWithPath) {
        if (!process.env.CONFIG_PATH) {
            console.warn("Internal error: process.env.CONFIG_PATH not set");
            return;
        }
        return path_1.default.resolve(process.env.CONFIG_PATH, fileWithPath);
    }
    ;
    /**
    ...
    async isReadable(filename) {
        try {
            await fs_extra_1.default.access(filename, fs_extra_1.default.constants.R_OK);
            return true;
        }
        catch (error) {
            return false;
    ...
    async readJson(filename) {
        // handle empty files
        const data = await fs_extra_1.default.readFile(filename, "utf-8");
     ...
     */
    async outputJson(fileWithPath, data, options) {
        ...
        if (!data ||
            ...
            await fs_extra_1.default.outputFile(fileWithPath, "");
        }
        else {
            options = options || { spaces: 2 };
            await fs_extra_1.default.outputJson(fileWithPath, data, options);
        }
    }
    ;
    /**
     ...
     */
    async readData(fileOrDir, params, config) {
        const stat = await fs_extra_1.default.stat(fileOrDir);
        if (stat.isDirectory()) {
     ...
     */
    async readFolder(folder, params, config) {
        const files = await fs_extra_1.default.readdir(folder, { withFileTypes: true });
        for (const file of files) {
            if (file.isDirectory()) {
                // recursively add subfolders using the subfolder name as key
                await this.readFolder(path_1.default.resolve(folder, file.name), [...params, file.name], config);
            }
            else if (file.isFile()) {
                const filename = file.name;
                const fileWithPath = path_1.default.resolve(folder, file.name);
                if (!filename) {
     ...
     * @param {string[]} params - the keys hierarchy under which data should be stored
     */
    async readFile(filename, params, config) {
        try {
...
;
exports.default = new DataExchangeUtil();
// JS SUPPORT
module.exports = new DataExchangeUtil();
//# sourceMappingURL=dataExchangeUtil.js.map