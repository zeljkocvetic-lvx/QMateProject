...
const dataExchangeUtil_1 = __importDefault(require("./dataExchangeUtil"));
const isBrowserDefined_1 = require("../hooks/utils/isBrowserDefined");
class DataExchange {
    /**
     * @function readParams
     ...
     */
    async readParams(config) {
        if (!config.params) {
            //nothing to do
            return;
        }
        if ((0, isBrowserDefined_1.isBrowserDefined)() && !browser.params) {
...
async function readImportParams(config, importParams) {
    // import
    ...
    for (let i = 0; i < params.length; i++) {
        ...
        if (isFileReadable) {
            // @ts-ignore
            await dataExchangeUtil_1.default.readData(fileOrDir, [param], config);
        }
        else {
...
async function readExportParams(config, exportParams) {
