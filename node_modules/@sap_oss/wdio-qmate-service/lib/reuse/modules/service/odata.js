...
class OData {
     ...
     * };
     */
    async patch(srv, entitySet, payload, headers) {
        if (!srv)
            throw new Error(SERVICE_INIT_ERROR);
        let entity = srv[entitySet];
        if (!entity)
     ...
     */
    async callFunctionImport(srv, functionImportName, options, raw = false) {
        if (!srv)
            throw new Error(SERVICE_INIT_ERROR);
        let functionImport = srv.functionImports[functionImportName];
        if (raw)
            functionImport = functionImport.raw();
