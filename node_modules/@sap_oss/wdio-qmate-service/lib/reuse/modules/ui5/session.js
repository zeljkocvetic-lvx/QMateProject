...
class Session {
    constructor() {
        this.vlf = new verboseLogger_1.VerboseLoggerFactory("ui5", "session");
     ...
     */
    async login(username, password, verify = false, timeout = parseFloat(process.env.QMATE_CUSTOM_TIMEOUT) || constants_1.GLOBAL_DEFAULT_WAIT_TIMEOUT) {
        ...
        try {
            await browser.waitUntil(async function () {
                const fioriForm = new Promise(async (res, rej) => {
                    try {
                ...
                const sapCloudForm = new Promise(async (res, rej) => {
                    try {
    ...
    async _clickSignOut(timeout = parseFloat(process.env.QMATE_CUSTOM_TIMEOUT) || constants_1.GLOBAL_DEFAULT_WAIT_TIMEOUT) {
